...
public interface UserChatRepository extends JpaRepository<UserChat, Integer> {
    ...
    @Query("select new FutureCraft.tikitaka.back_end.dto.object.UserDto(u.id, u.name, u.profileImage) from User u where u.id = :id and not exists (select uc from UserChat uc where uc.userId = u.id and uc.chatId = :chatId)")
    UserDto findByIdAndNotExistsFriendUser(@Param("id") String id, @Param("chatId") Integer chatId);

    boolean existsByChatIdAndUserId(Integer chatId, String participant);
}
