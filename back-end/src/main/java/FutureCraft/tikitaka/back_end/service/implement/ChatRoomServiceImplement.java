...
public class ChatRoomServiceImplement implements ChatRoomService {
    ...
    public ResponseEntity<? super ChatRoomCreateResponseDto> create(String id, ChatRoomCreateRequestDto requestDto) {
        try {
            ...
            for (UserEntity user : users) {
                ChatRoomParticipantEntity chatRoomParticipantEntity = new ChatRoomParticipantEntity(user.getId(), saved.getId());
                chatRoomParticipantRepository.save(chatRoomParticipantEntity);
            }
            
            ValueOperations<String, Object> ops = redisTemplate.opsForValue();
            List<SimpleUserDto> members = users.stream()
                .map(user -> new SimpleUserDto(user.getId(), user.getName(), user.getProfileImage())).collect(Collectors.toList());
            String key = "chatRoom:members:" + saved.getId();
            ops.set(key, members);

            for (SimpleUserDto member : members) {
                simpMessagingTemplate.convertAndSendToUser(member.getId(), "/queue/notify", new NotificationDto(NotificationType.CREATE_CHAT_ROOM, Integer.toString(saved.getId())));
            }
    ...
    public ResponseEntity<? super GetAddableListResponseDto> getAddableList(String id, Integer chatRoomId) {
        try {
            ...
            List<SimpleUserDto> result = list.stream().map((u) -> new SimpleUserDto(u.getId(), u.getName(), u.getProfileImage())).collect(Collectors.toList());
